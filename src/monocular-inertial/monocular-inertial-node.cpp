...
MonocularInertialNode::MonocularInertialNode()
  :   Node("ORB_SLAM3_ROS2")
{
  vocab_path = std::string(PROJECT_PATH) + "/vocabulary/ORBvoc.txt";
  camera_yaml_path = std::string(PROJECT_PATH) +
    "/config/monocular-inertial/RealSense_D435i.yaml";

  pSLAM = std::make_shared<ORB_SLAM3::System>(vocab_path, camera_yaml_path,
...
void MonocularInertialNode::SyncWithImu()
{
  while (rclcpp::ok()) {
    ...
    if (!imgBuf_.empty() && !imuBuf_.empty()) {
      ...
      try {
        pSLAM->TrackMonocular(imageFrame, tImage, vImuMeas);
